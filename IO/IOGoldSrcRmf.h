...
struct SRmfVisGroup
{
    ...
    IOCommon::SGoldSrcColor Color; // Border color in VHE of this group
    char Unknown;
...
struct SRmfFace
{
    ...
    std::vector<IOCommon::SGoldSrcVec3> Vertices; // Vertices data in clockwise
    IOCommon::SGoldSrcVec3 PlanePoints[3]; // Used for defining plane of this face. VHE directly use first 3 vertices rather than this;
...
struct SRmfObject
{
    ...
    IOCommon::SGoldSrcColor Color; // The display color in VHE.

    static std::shared_ptr<SRmfObject> create(std::ifstream& vFile);
protected:
...
struct SRmfEntity : SRmfObject
{
    ...
    std::vector<std::shared_ptr<SRmfSolid>> Solids;
    ...
    char Unknown2[14];
    IOCommon::SGoldSrcVec3 Origin; // Position of entity in world coordinates (only used for point entities)
    char Unknown3[4];
...
struct SRmfGroup : SRmfObject
{
    ...
    std::vector<std::shared_ptr<SRmfObject>> Objects;
...
struct SRmfCorner
{
    IOCommon::SGoldSrcVec3 Origin; // Position in world coordinates
...
struct SRmfCamera
{
    IOCommon::SGoldSrcVec3 EyePosition; // Eye position of the camera
    IOCommon::SGoldSrcVec3 Direction; // Direction of the camera
};
...
public:
    ...
    const std::shared_ptr<SRmfWorld>& getWorld() const;
...
private:
    ...
    std::shared_ptr<SRmfWorld> pWorld;
    int PathNum = 0;
