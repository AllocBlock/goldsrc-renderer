...
void CSceneObjWriter::addSceneInfo(sptr<SSceneInfo> vSceneInfo)
{
    for (size_t i = 0; i < vSceneInfo->pScene->getActorNum(); ++i)
    {
        ...
        sptr<SObjObject> pObjObject = make<SObjObject>();
...
void CSceneObjWriter::addObject(sptr<SObjObject> vObject)
{
...
{
    std::set<sptr<CIOImage>> ImageSet;
    uint64_t CurId = 1;
    ...
    for (sptr<SObjObject> pObject : m_ObjectSet)
    {
    ...
    for (sptr<CIOImage> pImage : ImageSet)
    {
