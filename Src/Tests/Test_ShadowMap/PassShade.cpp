...
std::vector<SLightPointData> readPointData(ptr<C3DObject> pObject)
{
    ...
    std::vector<SLightPointData> PointData(NumPoint);
...
void CRenderPassShade::setScene(const std::vector<ptr<C3DObject>>& vObjectSet)
{
    ...
    {
        std::vector<SLightPointData> PointData = readPointData(pObject);
        size_t SubBufferSize = sizeof(SLightPointData) * pObject->getVertexArray()->size();
