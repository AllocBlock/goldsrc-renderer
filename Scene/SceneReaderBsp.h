...
private:
    ...
    void __readTextures();
    std::vector<std::shared_ptr<C3DObjectGoldSrc>> __loadLeaf(size_t vLeafIndex);
    std::vector<std::shared_ptr<C3DObjectGoldSrc>> __loadEntity(size_t vModelIndex);
    void __loadBspTree();
    void __loadBspPvs();
    ...
    void __appendCube(glm::vec3 vOrigin, float vSize, std::shared_ptr<C3DObjectGoldSrc> voObject);
    ...
    void __appendBspFaceToObject(std::shared_ptr<C3DObjectGoldSrc> pObject, uint32_t vFaceIndex);
