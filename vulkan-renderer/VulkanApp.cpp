...
void                    CVulkanApp::initWindow()
{
    ...
    m_pWindow = glfwCreateWindow(1280, 720, "GoldSrc Renderer", nullptr, nullptr);                    // 创建窗口
...
void                    CVulkanApp::updateUniformBuffer(uint32_t vImageIndex)
{
    ...
    void* data;
    vkMapMemory(m_Device, m_UniformBuffersMemory[vImageIndex], 0, sizeof(Ubo), 0, &data);
    memcpy(data, &Ubo, sizeof(Ubo));
    vkUnmapMemory(m_Device, m_UniformBuffersMemory[vImageIndex]);
}
...
VkResult                CVulkanApp::__createDescriptorSetLayout()
{
    ...
    UboLayoutBinding.binding = 0; // 绑定位置，着色器对应
    UboLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER; // 指定类型
    UboLayoutBinding.descriptorCount = 1; // 可以是一个数组
    UboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT; // 阶段，这里是顶点着色器
...
VkResult                CVulkanApp::__createGraphicsPipeline()
{
    ...
    CreateInfoPipelineLayout.pSetLayouts = &m_DescriptorSetLayout;
...
/*******************************
* required: m_SwapChainImages
* assigned: m_UniformBuffers, m_UniformBuffersMemory
*/
void                CVulkanApp::__createUniformBuffers()
{
    ...
    for (size_t i = 0; i < m_SwapChainImages.size(); i++) {
        __createBuffer(m_UniformBuffers[i], m_UniformBuffersMemory[i], BufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    }
}

/*******************************
* required: m_SwapChainImages, m_Device
* assigned: m_DescriptorPool
*/
...
/*******************************
* required: m_SwapChainImages, m_DescriptorSetLayout, m_DescriptorPool, m_Device
* assigned: m_DescriptorSets
*/
VkResult                CVulkanApp::__createDescriptorSets()
{
